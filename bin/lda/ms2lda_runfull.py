        ...
        for f in [f for f in self.db_features if f.startswith(ftype)]:
            if f in other_names:
                ...
                if fmz <= other_max_mz[fpos]:
                    ...
                    self.augmented_features[f] = (fmz-self.bin_width/2,fmz+self.bin_width/2)
                    new_ones += 1
        print ("Finished matching ({ftype}). {exact_match} exact matches, {overlap_match} overlap matches, {new_ones} new features")
...
corpus = corpus[next(iter(corpus))]
